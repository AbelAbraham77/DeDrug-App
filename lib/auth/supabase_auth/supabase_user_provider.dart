...
class DeeDrugSupabaseUser extends BaseAuthUser {
  DeeDrugSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> deeDrugSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DeeDrugSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
